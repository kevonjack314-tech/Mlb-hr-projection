...
5b. **📋 Previous HRs** — the real home runs from recent days (live from **MLB
   StatsAPI box scores**: batter, HR count, lineup spot, and the starting pitcher
   they faced), with each bat's metrics, what the model rated them, and a
   "why they hit" insight; plus a per-lineup-spot breakdown.
...
#### ULX + the data — both, not either

The **ULX philosophy is live** alongside everything measured. `src/ulx.py` holds
the 9-point power checklist (Barrel% ≥ 8 · Hard-Hit% ≥ 40 · xSLG ≥ .450 · ISO ≥
.160 · Sweet-Spot% ≥ 30 · Avg EV ≥ 88 · Launch 10–28° · Pull% ≥ 35 · HR/FB ≥ 12)
and grades every bat 🟢 GREEN (≥7) / 🟡 YELLOW (4–6) / 🔴 RED (<4) — "bet the
profile, not the name". It feeds the Longshot score (12% weight), parlay role
fit, and the **ULX** / **ULX ✓** columns on every board.

The parlay builder carries **both compositions**:

| Strategy | 3-leg ticket | Comes from |
|---|---|---|
| `ulx` (default) | Anchor + Value + **Deep-Space Longshot** | the ULX playbook |
| `model` | Anchor + Value + Value | the graded record (longshot legs have under-performed) |

Tickets are graded on 15 checks — the 3 ULX role-composition checks plus 12
