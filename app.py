...
GLOSSARY = {
    ...
    "Barrel% 14d": "The hitter's barrel rate over the last 14 days (real, Statcast). Contact quality stabilizes far faster than HR rate, so this is a cleaner read on current form than a noisy 7-day HR rate.",
    "Barrel Trend": "Last-14-day barrel% minus the season baseline. Positive = the hitter's quality of contact is trending UP — the process heating up before the HRs show. Nudges the recent-form score.",
    "xwOBA Trend": "Last-14-day expected wOBA minus the season baseline. Positive = better contact quality lately, independent of luck. Pairs with the barrel trend as a stable 'getting hot' signal.",
    ...
    "Games in Row": "How many games this team has played on consecutive days with NO off day, ending today (schedule-derived). Bat speed and power sag with accumulated fatigue — 8+ in a row applies a small penalty. The hitter-side complement to pitcher velo decline; rarely modeled.",
...
DISPLAY_COLUMNS = {
    ...
    "barrel_pct_14": "Barrel% 14d",
    "barrel_trend": "Barrel Trend",
    "xwoba_trend": "xwOBA Trend",
...
COLUMN_CONFIG = {
    ...
    "Barrel% 14d": st.column_config.NumberColumn("Barrel% 14d", help=GLOSSARY["Barrel% 14d"], format="%.1f"),
    "Barrel Trend": st.column_config.NumberColumn("Barrel Trend", help=GLOSSARY["Barrel Trend"], format="%+.1f"),
    "xwOBA Trend": st.column_config.NumberColumn("xwOBA Trend", help=GLOSSARY["xwOBA Trend"], format="%+.3f"),
...
def _render_parlay(result, stake, key: str = "parlay"):
    ...
    # ULX role checks + data-driven quality checks.
    with st.expander(f"📋 ULX + quality checklist — {s['checks_passed']}/{s['checks_total']} · {light}", expanded=True):
        cc = st.columns(2)
...
def _hr_parlay_builder(df):
    st.caption(
        ...
        "archetypes, graded on the ULX checklist plus data-driven quality checks. "
    ...
    strategy = c2.selectbox(
        "Strategy",
        ["ulx", "model", "safe", "value", "boom"],
        format_func={"ulx": "ULX role-based", "model": "Model-driven (record)",
                     "safe": "Safest (highest prob)",
